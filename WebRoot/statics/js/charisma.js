...
//    $('#reply_createTime').val((d.getMonth()+1)+'/'+d.getDate()+'/'+d.getFullYear());
	...
	for(var i = 0;i<json.length;i++){
		...
		for(var j=0;j<json[i].subMenus.length;j++){
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\"/SLSaleSystem"+json[i].subMenus[j].funcUrl +"\"><i class="+pic+"></i><span class=\"hidden-tablet\">"+json[i].subMenus[j].functionName + "</span></a></li>";
		}
		result = result +"</ul></li>";
	}
	$("#menus").append(result);
	...
	{
		$('#bs-css').attr('href','/SLSaleSystem/statics/css/bootstrap-'+theme_name+'.css');
	}
