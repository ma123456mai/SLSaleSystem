		...
		success: function(result){
			if("failed" == result){
				...
				if(roleId == '2'){
					...
					$.post("/SLSaleSystem/backend/loadUserTypeList.html",{'s_role':roleId},function(result){
						if(result != ""){
...
$('.deluser').click(function(e){
	...
	if(confirm("您确定要删除【"+d_logincode+"】这个用户吗？")){
		//delete
		$.post("/SLSaleSystem/backend/deluser.html",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath,'delUserType':d_usertype},function(result){
			if("success" == result){
				alert("删除成功！");
...
{   
	if($("#"+t1+"").val() == '' || $("#"+t1+"").val() == null){
		...
		$.ajaxFileUpload
	    ({ 
	           url:'/SLSaleSystem/backend/upload.html', //处理上传文件的服务端
	           secureuri:false,
	           fileElementId:t1,
	           dataType: 'json',
