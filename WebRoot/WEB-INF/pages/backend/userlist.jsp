...
<div>
	<ul class="breadcrumb">
		...
		<li>
			<a href="/SLSaleSystem/backend/userlist.html">用户管理</a>
		</li>
	</ul>
</div>
...
<div class="row-fluid sortable">		
 ...
 <form action="/SLSaleSystem/backend/userlist.html" method="post">
	...
	<div class="pagination pagination-centered">
	  ...
	  <c:choose>
	  	...
	  	<c:otherwise>
	  	<li><a href="/SLSaleSystem/backend/userlist.html?currentpage=1&s_loginCode=${s_loginCode}&s_referCode=${s_referCode}&s_roleId=${s_roleId}&s_isStart=${s_isStart}" title="首页">首页</a></li>
	  	</c:otherwise>
	  </c:choose>
		<c:if test="${page.prevPages!=null}">
			<c:forEach items="${page.prevPages}" var="num">
				<li><a href="/SLSaleSystem/backend/userlist.html?currentpage=${num}&s_loginCode=${s_loginCode}&s_referCode=${s_referCode}&s_roleId=${s_roleId}&s_isStart=${s_isStart}"
		...
		<c:if test="${page.nextPages!=null}">
			<c:forEach items="${page.nextPages}" var="num">
				<li><a href="/SLSaleSystem/backend/userlist.html?currentpage=${num}&s_loginCode=${s_loginCode}&s_referCode=${s_referCode}&s_roleId=${s_roleId}&s_isStart=${s_isStart}" title="${num}">
				${num} </a></li>
			</c:forEach>
		</c:if>
		<c:if test="${page.pageCount !=null}">
			<c:choose>
		  	...
		  	<c:otherwise>
		  	<li><a href="/SLSaleSystem/backend/userlist.html?currentpage=${page.pageCount}&s_loginCode=${s_loginCode}&s_referCode=${s_referCode}&s_roleId=${s_roleId}&s_isStart=${s_isStart}" title="尾页">尾页</a></li>
		  	</c:otherwise>
		    </c:choose>
	    </c:if>
		<c:if test="${page.pageCount == null}">
...
<form action="/SLSaleSystem/backend/adduser.html" enctype="multipart/form-data" method="post" onsubmit="return addUserFunction();">
		...
		<ul class="topul">
			...
			<li>
			  ...
			  <select id="selectusertype" name="userType" style="width:100px;">
	 			<option value="" selected="selected">--请选择--</option>
	 			<c:if test="${memberTypeList != null}">
	 				<c:forEach items="${memberTypeList}" var="member">
	 					<option value="${member.id}">${member.valueName}</option>
	 				</c:forEach>
	 			</c:if>
	 		 </select>
			</li>
...
<form action="/SLSaleSystem/backend/modifyuser.html" enctype="multipart/form-data" method="post" onsubmit="return modifyUserFunction();">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal">×</button>
		<h3>修改用户信息</h3>
	</div>
...
<script type="text/javascript" src="/SLSaleSystem/statics/localjs/userlist.js"></script>